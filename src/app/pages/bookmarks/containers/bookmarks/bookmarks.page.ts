...
export class BookmarksPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.bookmarks$ = this.store.pipe(select(fromBookmarksSelectors.selectBookmarksList));//selecionando bookmark do state

    this.searchTypeaheadControl.valueChanges //retorna CityTypeaheadItem quando o usuário seleciona
      .pipe(takeUntil(this.componentDestroyed$))
      .subscribe((value: CityTypeaheadItem) =>
        this.store.dispatch(fromBookmarksActions.toggleBookmarById({ id: value.geonameid }))//dispara função para adicionar essa cidade aos favoritos
      );
  }
  ...
  removeBookmark(id: number) {
    this.store.dispatch(fromBookmarksActions.removeBookmark({ id }));//dispara função para remover bookmark
  }
}
